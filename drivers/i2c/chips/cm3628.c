...
#if (POLLING_PROXIMITY == 1)
#define POLLING_DELAY		300
#define FIRST_POLLING_DELAY		200
#define TH_ADD			3
static int recode_pre_status = -1;
int enable_poll_adc = 0;
int change_thd_flg = 0;
int first_change_thd = 0;
#endif
...
static struct mutex als_enable_mutex, als_disable_mutex, als_get_adc_mutex, ps_thd_mutex;
...
static int I2C_RxData(uint16_t slaveAddr, uint8_t *rxData, int length)
{
	...
	for (loop_i = 0; loop_i < I2C_RETRY_COUNT; loop_i++) {
		...
		D("[CM3628] %s, i2c err, slaveAddr 0x%x ISR gpio %d  = %d \n",
...
static int I2C_TxData(uint16_t slaveAddr, uint8_t *txData, int length)
{
	...
	for (loop_i = 0; loop_i < I2C_RETRY_COUNT; loop_i++) {
		...
		val = gpio_get_value(lpi->intr_pin);
		/*check intr GPIO when i2c error*/
		D("[CM3628 error] %s, i2c err, slaveAddr 0x%x, register 0x%x, value 0x%x, ISR gpio%d  = %d\n",
				__func__, slaveAddr, txData[0], txData[1], lpi->intr_pin, val);
...
static int get_ls_adc_value(uint16_t *als_step, bool resume)
{
	...
	*als_step |= (uint16_t)lsb;
	...
	D("[CM3628] %s: raw *als_step = 0x%X, lpi->ls_calibrate = %d\n",
...
/*static int set_psensor_range(uint8_t ps_thd_set)
{
	int ret = 0;
	struct cm3628_info *lpi = lp_info;
...
static void report_psensor_input_event(struct cm3628_info *lpi)
{
	uint8_t ps_data, add = 0, ps_data_ref;
	int val, ret = 0;
	int count = 0;
	...
	if (lpi->mfg_mode != NO_IGNORE_BOOT_MODE) {
		mutex_lock(&ps_thd_mutex);
		if (change_thd_flg == 1) {
			D("[CM3628 ] because proximity reset, wait 100ms");
			while (gpio_get_value(lpi->intr_pin) == 0) {
				_cm3628_I2C_Read_Byte(lpi->check_interrupt_add, &add);
				count++;
				D("[CM3628 err] because proximity reset, read interrupt 0x%x", add);
				if (count == 20)
					break;
			}
			ret = get_ps_adc_value(&ps_data_ref);/*check i2c result*/
			if (ret != 0) {
				ps_data_ref = 0;
				D("[CM3628 err]  proximity read err");
			}
			msleep (50);
		}
	}
	...
	if (lpi->mfg_mode != NO_IGNORE_BOOT_MODE) {
		if (change_thd_flg == 1) {
			D("[CM3628 ]  proximity ps_data_ref 0x%x, adc 0x%x", ps_data_ref, ps_data);
			ps_data = (ps_data + ps_data_ref)/2;
		}
	}
#endif
	...
	if (ret == 0) {
		...
		D("[CM3628] proximity %s, adc=0x%x, ps_thd 0x%x\n", val ? "FAR" : "NEAR", ps_data, lpi->ps_thd_set);
		...
		if (lpi->mfg_mode != NO_IGNORE_BOOT_MODE) {
			...
			while (gpio_get_value(lpi->intr_pin) == 0) {
					_cm3628_I2C_Read_Byte(lpi->check_interrupt_add, &add);
					count++;
					D("[CM3628 err] proximity i2c err, read interrupt 0x%x", add);
					if (count == 20)
						break;
			}
			msleep (50);
		}
		D("[CM3628 err] proximity i2c err, retry\n");
		ret = get_ps_adc_value(&ps_data);/*check i2c result*/
#endif
		if (ret == 0) {
			val = (ps_data >= lpi->ps_thd_set) ? 0 : 1;
			...
			val = 1;
			ps_data = 0;
			D("[CM3628 err] proximity i2c err, report %s, adc=%d\n", val ? "FAR" : "NEAR", ps_data);
		}
	}
#if (POLLING_PROXIMITY == 1)
	if (lpi->mfg_mode != NO_IGNORE_BOOT_MODE && enable_poll_adc == 1) {

		if (recode_pre_status == -1)
			recode_pre_status = val;
		else {
			if (recode_pre_status == val && val == 0)
				D("[CM3628 ] proximity repeat near status");
			else if (recode_pre_status == val && val == 1)
				D("[CM3628 ] proximity repeat far statusr");
	...
	if (lpi->mfg_mode != NO_IGNORE_BOOT_MODE) {
		if (change_thd_flg == 1)
			change_thd_flg = 0;
		mutex_unlock(&ps_thd_mutex);
	}
#endif

	if (time_before(lpi->j_end, (lpi->j_start + NEAR_DELAY_TIME)) &&
		(lpi->mfg_mode != NO_IGNORE_BOOT_MODE)) {
		D("[CM3628] Ignore NEAR event, enable pocket_mode\n");
...
static void sensor_irq_do_work(struct work_struct *work)
{
	...
	/*check ALS or PS*/
	_cm3628_I2C_Read_Byte(lpi->check_interrupt_add, &add);
	/*D("[CM3628] %s: check_interrupt_add = 0x%03X, add = 0x%x \n",
		__func__, lpi->check_interrupt_add, add);*/
...
static int get_stable_ps_adc_value(uint8_t *ps_adc)
{
	...
	for (i = 0; i < 1; i++) {
		while (gpio_get_value(lpi->intr_pin) == 0) {/*wait interrupt GPIO high*/
			msleep(1);
			wait_count++;
			if (wait_count > 5) {
				pr_err("[CM3628 error]%s: interrupt GPIO low, get_ps_adc_value\n",
				__func__);
				return -EIO;
			}
		}
		...
		if (wait_count < 60/1) {//wait gpio less than 60ms
			msleep(60 - (1*wait_count));
		}
		wait_count = 0;*/
	}
	*ps_adc = value[0]& 0xFF;
	/*D("Sta_ps: Before sort, value[0, 1] = [0x%x, 0x%x], ave = 0x%x",
		value[0], value[1], *ps_adc) ;*/
#if (0)
	...
	for (i = lpi->ps_base_index; i >= 1; i--) {
		if (ps_adc > lpi->mapping_table[i])
			...
			if (i == (lpi->mapping_size - 1))
				...
				ps_thd = (lpi->mapping_table[i] +
						   TH_ADD);
			mutex_lock(&ps_thd_mutex);
			if (recode_pre_status == 1 || first_change_thd == 0) {/*only far, could change */
				/* settng command code(0x01) = 0x03*/
				ret = _cm3628_I2C_Write_Byte(lpi->PS_slave_address,
						PS_thd, ps_thd);
				if (ret == 0) {
					first_change_thd = 1;
					change_thd_flg = 1;
					lpi->ps_thd_set = ps_thd;
					lpi->ps_base_index = (i-1);
					D("[CM3628] SET THD: lpi->ps_thd_set = 0x%x\n",
							lpi->ps_thd_set);
				}
				else
					D("[CM3628 err] SET THD fail: lpi->ps_thd_set = 0x%x, change ps_thd 0x%x\n",
							lpi->ps_thd_set, ps_thd);
			}
			mutex_unlock(&ps_thd_mutex);
			break;
...
static int psensor_enable(struct cm3628_info *lpi)
{
	...
	if (lpi->mfg_mode != NO_IGNORE_BOOT_MODE) {
		...
		first_change_thd = 0;
		msleep(50);
		ret = get_ps_adc_value(&ps_adc);
		D("[CM3628] INITIAL ps_adc = 0x%02X, ret = %d", ps_adc, ret);
		if ((ret == 0) && (lpi->mapping_table != NULL) &&
			((ps_adc >= lpi->ps_thd_set - 1))) {
			enable_poll_adc = 1;
			change_thd_flg = 0;
			queue_delayed_work(lpi->lp_wq, &polling_work,
				msecs_to_jiffies(FIRST_POLLING_DELAY));
		}
	}
#endif
	mutex_lock(&ps_thd_mutex);
	cmd = (lpi->ps_it | CM3628_PS_INT_EN);
	enable_ps_int(cmd);
	lpi->j_start = jiffies;
	mutex_unlock(&ps_thd_mutex);
	D("[CM3628] %s: j_start = %lu, enable P-sensor interrupt", __func__, lpi->j_start);
...
static int psensor_disable(struct cm3628_info *lpi)
{
	...
	ret = _cm3628_I2C_Write_Byte(lpi->PS_slave_address,
			PS_cmd_cmd, lpi->ps_it|CM3628_PS_SD);
	if (ret < 0) {
...
void psensor_set_kvalue(struct cm3628_info *lpi)
{
	...
	if (ps_kparam1 >> 16 == PS_CALIBRATED) {
		...
		if (iteration == 10)
			lpi->ps_it = CM3628_PS_IT_1T;
		else if (iteration == 16)
			lpi->ps_it = CM3628_PS_IT_1_6T;
		else /* default 1.3T */
			lpi->ps_it = CM3628_PS_IT_1_3T;

		D("[CM3628] %s: PS calibrated ps_it = 0x%x\n",
				__func__, lpi->ps_it);
		...
		lpi->ps_it = CM3628_PS_IT_1T;/*defult  if no calibration*/
			...
			struct device_attribute *attr, char *buf)
{
	...
	D("[CM3628] %s, ADC 0x%x, ps_pocket_mode %d\n",
		__func__, value, lpi->ps_pocket_mode);
				...
				const char *buf, size_t count)
{
	...
	if (ps_en) {
		if (ps_en == 10)
			...
			lpi->ps_it = CM3628_PS_IT_1_6T;
		D("[CM3628] %s: ps_en=%d, ps_it=0x%x\n",
			__func__, ps_en, lpi->ps_it);
		psensor_enable(lpi);
	} else
		psensor_disable(lpi);
...
static DEVICE_ATTR(ps_parameters, 0666,
				...
				const char *buf, size_t count)
{
	...
	D("[CM3628]%s: store value = 0x%X, 0x%X\n",
	...
	if (value == 10)
		...
		lpi->ps_it = CM3628_PS_IT_1_3T;
	D("[CM3628]%s: lpi->ps_it =0x%x\n", __func__, lpi->ps_it);
	if (lpi->ps_enable)
		enable_ps_int(lpi->ps_it | CM3628_PS_INT_EN);

	lpi->inte_cancel_set = (param2 & 0xFF);
	psensor_intelligent_cancel_cmd(lpi);

	D("[CM3628]%s: lpi ->inte_cancel_set = 0x%X\n",
		__func__, lpi->inte_cancel_set);
...
static DEVICE_ATTR(ps_conf2, 0666, ps_conf2_show, ps_conf2_store);
...
static DEVICE_ATTR(ps_conf1, 0666, ps_conf1_show, ps_conf1_store);
...
static DEVICE_ATTR(ps_thd, 0666, ps_thd_show, ps_thd_store);
...
static DEVICE_ATTR(ps_canc, 0666, ps_canc_show, ps_canc_store);
#if 0
			...
			struct device_attribute *attr, char *buf)
{
	...
	buffer[0] = CM3628_CONFIGURE;
	ret = I2C_RxData(buffer, 1);
	if (ret < 0) {
		pr_err("[CM3628 error]%s: I2C_RxData fail\n", __func__);
		return ret;
	}
	value = buffer[0];
	D("[CM3628] %s: Configure reg = 0x%x\n", __func__, value);

	lpi->led = (value & 0x08) >> 3;
	D("[CM3628] %s: lpi->led = 0x%x\n", __func__, lpi->led);
				...
				const char *buf, size_t count)
{
	...
	D("[CM3628] %s: store value = %d\n", __func__, param);

	if (param == 0 || param == 1) {
		ret = _cm3628_set_reg_bit(lpi->i2c_client,
			param, CM3628_CONFIGURE, CM3628_PROX_DR);
		if (ret < 0) {
		...
		D("[CM3628] %s: Set PS LED ERR, use 0 or 1!\n",
			__func__);
				...
				const char *buf, size_t count)
{
	...
	if (kadc_temp <= 0 || lpi->golden_adc <= 0) {
		printk(KERN_ERR "[CM3628] %s: kadc_temp=0x%x, als_gadc=0x%x\n",
	...
	lpi->als_gadc = lpi->golden_adc;
	printk(KERN_INFO "[CM3628] %s: als_kadc=0x%x, als_gadc=0x%x\n",
			__func__, lpi->als_kadc, lpi->als_gadc);

	if (lightsensor_update_table(lpi) < 0)
		printk(KERN_ERR "[CM3628] %s: update ls table fail\n", __func__);
	mutex_unlock(&als_get_adc_mutex);
	return count;
}
				...
				const char *buf, size_t count)
{
	...
	for (i = 0; i < 10; i++) {
		...
		if (tempdata[i] < 1 || tempdata[i] > 0xffff) {
			printk(KERN_ERR
			"[CM3628 Err]adc_table[%d] =  0x%x Err\n",
			i, tempdata[i]);
			return count;
		}
	}
...
static DEVICE_ATTR(ls_adc_table, 0666,
...
static DEVICE_ATTR(ls_conf1, 0666, ls_conf1_show, ls_conf1_store);
...
static DEVICE_ATTR(ls_conf2, 0666, ls_conf2_show, ls_conf2_store);
